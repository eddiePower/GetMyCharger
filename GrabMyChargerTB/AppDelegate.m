...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    self.locationManager = mainViewController.locationManager;
   */
...
- (void)applicationDidEnterBackground:(UIApplication *)application
{
    ...
    //grab the app from the sharedApplication object.
    ...
    //create timer to restart the location updates after 9.1 mins or 550 seconds.
    self.silenceTimer = [NSTimer scheduledTimerWithTimeInterval:550 target:self
                                                       selector:@selector(reloadLocationManager)
                                                       userInfo:nil repeats:YES];
    //get the latest location after location reboot.
    [self updateLocation];
...
-(void)batteryStateChanged:(NSNotification*)notification
{
    ...
    //debug state
    NSLog(@"The battery state has changed to %ld", [UIDevice currentDevice].batteryState);
    ...
    if ([UIDevice currentDevice].batteryState == 1)
    {
        ...
        if (self.inBackground == 1)
        {
            ...
            if (self.lastBatteryState != [UIDevice currentDevice].batteryState)
            {
                ...
                [notification setAlertBody:@"Don't forget to grab your charger!"];
                [notification setFireDate:[NSDate dateWithTimeIntervalSinceNow:0]];
                [notification setTimeZone:[NSTimeZone defaultTimeZone]];
                [notification setSoundName:UILocalNotificationDefaultSoundName];
    ...
    else if ([UIDevice currentDevice].batteryState == 2)
    {
            ...
            {
                //Application is running no need to notify except a alertBox View.
    ...
    else if ([UIDevice currentDevice].batteryState == 3)
    {
       ...
       if (self.inBackground == 1)
       {
           if (self.lastBatteryState != [UIDevice currentDevice].batteryState)
           {
              UILocalNotification* notification = [[UILocalNotification alloc] init];
              [notification setCategory:@"ACCEPT_CATEGORY"];
              [notification setAlertBody:@"Your battery is now fully charged dont forget your charger if your unplugging!"];
              [notification setFireDate:[NSDate dateWithTimeIntervalSinceNow:0]];
              [notification setTimeZone:[NSTimeZone defaultTimeZone]];
              [notification setSoundName:UILocalNotificationDefaultSoundName];
      ...
      {
          //Application is running no need to notify
          
      }
    }//end else if
    else  //catch the unknowen battery state stuff.
    {
